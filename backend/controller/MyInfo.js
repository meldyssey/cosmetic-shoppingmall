        ...
        try {
            if (action === 'getUserInfo') {
                ...
                try {
                    ...
                    if (!email) {
                        return res.status(400).json({ error: '탈퇴 처리할 이메일이 없습니다.' });
                    }
                    ...
                    if (!customer.length) {
                        return res.status(404).json({ error: '해당 이메일의 회원 정보를 찾을 수 없습니다.' });
                    }
                    ...
                    const insertQuery = `
                        ...
                        FROM customers
                        WHERE customer_id = ?`;
                    const [insertResult] = await db.query(insertQuery, [customer_id]);
                    console.log(`deleted_customers 삽입 결과: ${insertResult.affectedRows}`); // 디버깅 로그
            
                    if (insertResult.affectedRows === 0) {
                        return res.status(500).json({ error: '탈퇴 고객 데이터 이동 실패.' });
                    }
            
                    // auth 테이블에서 삭제
                    const deleteAuthQuery = 'DELETE FROM auth WHERE customer_id = ?';
                    const [deleteAuthResult] = await db.query(deleteAuthQuery, [customer_id]);
                    console.log(`auth 삭제 결과: ${deleteAuthResult.affectedRows}`); // 디버깅 로그
            
                    // customer 테이블에서 삭제
                    const deleteCustomerQuery = 'DELETE FROM customers WHERE customer_id = ?';
                    const [deleteCustomerResult] = await db.query(deleteCustomerQuery, [customer_id]);
                    console.log(`customers 삭제 결과: ${deleteCustomerResult.affectedRows}`); // 디버깅 로그
            
                    if (deleteAuthResult.affectedRows === 0 || deleteCustomerResult.affectedRows === 0) {
                        return res.status(500).json({ error: '회원 데이터 삭제 실패.' });
                    }
            
                    res.json({ success: true, message: '회원 탈퇴가 완료되었습니다.' });
                } catch (err) {
                    console.error('회원 탈퇴 오류:', err); // 에러 로그
                    return res.status(500).json({ error: '탈퇴 진행 중 서버 오류가 발생했습니다.' });
                }
            }
             else {
